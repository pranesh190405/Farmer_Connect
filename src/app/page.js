      ...
      <section className="relative bg-green-50 pt-20 pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up delay-300">
            <Link href="/farmer/register" className="group bg-green-600 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-green-700 transition-all shadow-lg hover:shadow-green-200 flex items-center justify-center gap-2">
              {t('landing.buttons.startSelling')}
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Link>
            <Link href="/buyer/register" className="group bg-white text-gray-900 border border-gray-200 px-8 py-4 rounded-xl font-bold text-lg hover:border-green-200 hover:bg-green-50 transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div className="p-8 rounded-3xl bg-gray-50 border border-gray-100 hover:border-green-100 transition-colors group">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('landing.features.fresh.title')}</h3>
              <p className="text-gray-500 leading-relaxed">{t('landing.features.fresh.desc')}</p>
            </div>
            <div className="p-8 rounded-3xl bg-gray-50 border border-gray-100 hover:border-green-100 transition-colors group">
              <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-500">
                <ShieldCheck className="w-7 h-7 text-green-600" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('landing.features.quality.title')}</h3>
              <p className="text-gray-500 leading-relaxed">{t('landing.features.quality.desc')}</p>
            </div>
            <div className="p-8 rounded-3xl bg-gray-50 border border-gray-100 hover:border-green-100 transition-colors group">
              <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-500">
                <TrendingUp className="w-7 h-7 text-green-600" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('landing.features.fair.title')}</h3>
              <p className="text-gray-500 leading-relaxed">{t('landing.features.fair.desc')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-white border-t py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-6 text-sm text-gray-500 font-medium">
            <Link href="/admin/login" className="hover:text-green-600 transition-colors">{t('dashboard.adminConsole')}</Link>
            <Link href="/profile" className="hover:text-green-600 transition-colors">{t('nav.profile')}</Link>
            <Link href="/market" className="hover:text-green-600 transition-colors">{t('nav.market')}</Link>
          </div>
