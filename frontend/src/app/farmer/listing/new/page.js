...
export default function NewListingPage() {
    ...
    const { isApproved, status } = useAuth(); // Get approval status
    ...
    if (!isApproved) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
                <div className="bg-yellow-50 p-6 rounded-2xl max-w-md border border-yellow-200">
                    <h2 className="text-xl font-bold text-yellow-800 mb-2">Account Pending Verification</h2>
                    <p className="text-yellow-700">
                        Your account status is currently <strong>{status}</strong>.
                        Please wait for an admin to verify your account before you can create listings.
                    </p>
                    <Link href="/farmer/dashboard" className="mt-6 inline-block text-yellow-900 underline">
                        Back to Dashboard
                    </Link>
                </div>
            </div>
